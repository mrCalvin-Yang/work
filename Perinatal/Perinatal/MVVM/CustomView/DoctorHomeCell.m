...
-(void)addtqStartView{
    self.tqStartView = [[TQStarRatingView alloc] initWithFrame:CGRectMake(90, 111, 88, 17) numberOfStar:5];
    [self.tqStartView setScore:0.4f withAnimation:YES];
    [self.contentView addSubview:self.tqStartView];
}

-(void)addAskLabel{
    self.askLabel = [[UILabel alloc] init];
    self.askLabel.font = H12;
    self.askLabel.textAlignment = NSTextAlignmentRight;
    self.askLabel.text = @"122个问答";
    self.askLabel.textColor = self.skillLabel.textColor;
...
-(void)addConstraints{
    ...
    [self.skillLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.left.equalTo(self.topImageView.mas_right).with.offset(padding15);
        make.bottom.equalTo(self.tqStartView.mas_top).with.offset(-8);
